...
import { createUserFriendlyError } from '../routing/model-config';
...
export class GoogleAdapter extends BaseProviderAdapter {
  ...
  private buildContents(prompt: string, systemPrompt?: string, history?: MessageData[]): GoogleContent[] {
    ...
    // Helper to convert text with potential data URIs into Gemini parts
    const textToParts = (text: string): any[] => {
      const parts: any[] = [];
      
      // Look for data URIs (images and PDFs)
      // Regex detects: data:image/png;base64,xxxx or data:application/pdf;base64,xxxx
      const dataUriRegex = /data:(image\/[a-z]+|application\/pdf);base64,([a-zA-Z0-9+/=]+)/g;
      ...
      // Fallback if no data URIs found or if empty
      if (parts.length === 0) {
